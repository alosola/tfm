...
class Stokes:
    ...
    def _plot_frame(self, data, wave_to_plot):
        ...
        if hasattr(self, 'wave_array'):
            if (wave_to_plot < 112):
                ...
                title = self.name + " data, for wavelength " + str(round(self.wave_array[wave_to_plot],3)) + " in frame " + str(wave_to_plot)
                ...
                title = self.name + " data, for wavelength " + str(round(self.wave_array[index],3)) + " in frame " + str(index)
                fig, _ = plot_data(data[:,:,index], title)
        else:
            try:
                # If the wavelength hasn't been calibrated, you are forced to select only the index
                title = self.name + " data, for wavelength index " + str(wave_to_plot)
                fig, _ = plot_data(data[:,:,wave_to_plot], title)
            except:
